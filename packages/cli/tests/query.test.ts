...
describe("defaultFileName", () => {
  ...
  it("includes the repository for a pin", () => {
    ...
    expect(
      pin &&
        defaultFileName(pin, { username: "anuraghazra", repo: "convoychat" }),
    ).toBe("pin-anuraghazra-convoychat.svg");
  });

  it("keeps a gist id, which has no username", () => {
    const gist = findCard("gist");
    expect(gist && defaultFileName(gist, { id: "bbfce31e" })).toBe(
      "gist-bbfce31e.svg",
    );
  });
  ...
  it("writes a name a shell can handle", () => {
    const stats = findCard("stats");
    expect(stats && defaultFileName(stats, { username: "a b/c" })).toBe(
      "stats-a-b-c.svg",
    );
  });
});
...
describe("the card catalog", () => {
  ...
  it("names every option once per card", () => {
    for (const card of cards) {
      const names = [...card.required, ...card.options].map(
        (option) => option.name,
      );
      expect(new Set(names).size, `${card.id} repeats an option`).toBe(
        names.length,
      );
    }
  });

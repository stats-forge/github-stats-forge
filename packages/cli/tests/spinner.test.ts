...
describe(withSpinner, () => {
  it('answers with what the work answered', async () => {
    ...
    await expect(withSpinner('Rendering', () => Promise.resolve('the card'), stream)).resolves.toBe(
      'the card',
    );
  });
  ...
  it('draws a frame beside the label while it waits', async () => {
    ...
    const done = withSpinner('Rendering', () => vi.advanceTimersByTimeAsync(200), stream);
    await done;

    const frames = stream.written.filter((chunk) => chunk.includes('Rendering'));

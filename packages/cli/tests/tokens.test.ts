...
describe("tokensFromEnv", () => {
  it("reads the PAT_n names core writes", () => {
    expect(
      tokensFromEnv({ PAT_1: "one", PAT_2: "two", HOME: "/somewhere" }),
    ).toStrictEqual([
  ...
  it("ignores a name that only looks like one", () => {
    expect(
      tokensFromEnv({ PAT: "x", PAT_ONE: "y", MY_PAT_1: "z" }),
    ).toStrictEqual([]);
  });
});
...
describe("loadEnvFile", () => {
  ...
  it("passes over a default file that is not there", () => {
    expect(loadEnvFile(join(tmpdir(), "no-such-dir", ".env"), false)).toBe(
      false,
    );
  });
  ...
  it("says so when a named file is not there", () => {
    expect(() =>
      loadEnvFile(join(tmpdir(), "no-such-dir", ".env"), true),
    ).toThrow(/No env file at/);
  });
});
